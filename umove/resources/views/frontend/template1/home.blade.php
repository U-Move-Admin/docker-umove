...
@section('title', config('app.name').': '.config('broker.home_title'))
@section('meta')
    <meta name="description" content="{{ config('broker.home_desc').' | '.config('app.name') }}" />

    <!-- facebook --> 
    <meta property="og:type" content="article">
    <meta property="og:url" content="{{ url()->current() }}" />
    <meta property="og:title" content="{{ config('broker.name').': '.config('broker.home_title') }}" />
    <meta property="og:description" content="{{ config('broker.home_desc').' | '.config('broker.name') }}">  
    <meta property="og:image" content="{{ asset(config('broker.logo')) }}">
    
    <!-- twitter -->
    <meta name="twitter:title" content="{{ config('broker.name').': '.config('broker.home_title') }}">  
    <meta name="twitter:description" content="{{ config('broker.home_desc').' | '.config('broker.name') }}">  
    <meta name="twitter:image:src" content="{{ asset(config('broker.logo')) }}">  
    
    <!-- google -->
    <meta itemprop="name" content="{{ config('broker.name').': '.config('broker.home_title') }}">  
    <meta itemprop="description" content="{{ config('broker.home_desc').' | '.config('broker.name') }}">  
    <meta itemprop="image" content="{{ asset(config('broker.logo')) }}"> 
    
    <link rel="canonical" href="{{ url()->current() }}">    
@endsection
@section('content')
    @include('frontend.template1.elements.top_menu')
    @include('frontend.template1.elements.header')
    @include('frontend.template1.elements.search_home')
                ...
                <div class="col-lg-4 col-md-6 col-sm-6 wow fadeInUp delay-04s" style="visibility: visible; animation-name: fadeInUp;">
                    <div class="media services-info">
                        ...
                        <div class="media-body">
                            ...
                            <a class="text-primary" href="{{ url('/services') }}">Go to Services</a>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-12 filtr-item" data-category="1">
                        <div class="property-box">
                            <div class="property-thumbnail">
                                <a href="{{ asset('/detail/'.$property['furl'])}}" class="property-img">
                                    <div class="price-ratings-box">
                                        <p class="price">
                                          £{{$property['price']}}
                                        </p>
                                    </div>
                                    <img class="img-fluid" src="{{ !empty($property['images'][0]['image_name']) ? asset('/img/uploads/'.$property['id'].'/'.$property['images'][0]['image_name']) : asset('/frontend/templates/img/noimage.png')}}" alt=""> 
                                </a>
                                <div class="property-overlay">
                                    <a href="{{ asset('/detail/'.$property['furl']) }}" class="overlay-link">
                                        <i class="fa fa-link"></i>
                                    </a>
                                    <div class="property-magnify-gallery">
                                        @if(!empty($property['images'][0]['image_name']))
                                        <a href="{{ asset('/img/uploads/'.$property['id'].'/'.$property['images'][0]['image_name'])}}" class="overlay-link">
                                            <i class="fa fa-expand"></i>
                                        </a>
                                        <a href="{{ asset('/img/uploads/'.$property['id'].'/'.$property['images'][0]['image_name']) }}" class="hidden"></a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="detail">
                                <h1 class="title">
                                    <a href="{{ asset('/detail/'.$property['furl'])}}">{{ Str::title($property['title'])}}</a>
                                </h1>
