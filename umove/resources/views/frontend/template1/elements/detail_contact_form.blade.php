    ...
    <form method="post" action="{{ route('contact') }}" id="contact_form" >
        ...
        <div class="form-group">
            <input type="text" 
                ...
                class="form-control" 
                placeholder="Name"
                value="{{ Auth::check() && Auth::user()->getRoleNames()->count() == 0 ? Auth::user()->name : null }}"
                required>
        ...
        <div class="form-group">
            <input 
                ...
                class="form-control" 
                placeholder="Phone" 
                value="{{ Auth::check() && Auth::user()->getRoleNames()->count() == 0 ? Auth::user()->tel : null }}"
                required>
        </div>
        <div class="form-group">
            <input 
                ...
                class="form-control" 
                placeholder="Email" 
                value="{{ Auth::check() && Auth::user()->getRoleNames()->count() == 0 ? Auth::user()->email : null }}"
                required>
        </div>
        <div class="form-group">
            <textarea required name="text" class="form-control" rows="5" >Hello, I am interested in {{$title}}</textarea>
        </div>
...
<script>
    async function handleMessage(e) {
        ...
        if(name != '' && phone != '' && email != '' && text != '' && customer_type != '') {
            ...
            try {
                const response = await fetch("{{ route('send_property_message') }}", {
                    method: "post",
                    headers: {
